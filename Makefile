...
install: build
	...
	@if [ "$$(id -u)" != "0" ]; then \
		...
		sudo chgrp log /var/log/faceid.log 2>/dev/null || sudo chgrp adm /var/log/faceid.log 2>/dev/null || true; \
		sudo chmod 664 /var/log/faceid.log; \
	else \
		touch /var/log/faceid.log; \
		chgrp log /var/log/faceid.log 2>/dev/null || chgrp adm /var/log/faceid.log 2>/dev/null || true; \
		chmod 664 /var/log/faceid.log; \
	fi
