...
enum class DetectionModelType {
    ...
    RETINAFACE,   // RetinaFace (fallback): embedded, robust in difficult conditions
    UNKNOWN
};
...
public:
    ...
    // Load models - now uses embedded models in addition to filesystem
    // - Recognition model: loads from CONFIG_DIR/models/ (user's enrolled model)
    // - Detection models: YuNet (primary) and RetinaFace (fallback) embedded in binary
    // model_base_path: optional path for recognition model (empty = auto-detect from models dir)
    ...
    // Helper: Parse param file to extract output dimensions (for recognition model)
...
private:
    ...
    ncnn::Net yunet_net_;         // YuNet detection model (embedded - primary)
    ncnn::Net retinaface_net_;    // RetinaFace detection model (embedded - fallback)
    
    bool models_loaded_ = false;
    bool detection_model_loaded_ = false;   // YuNet (embedded - always true after loadModels)
    bool detection2_model_loaded_ = false;  // RetinaFace (embedded - always true after loadModels)
    
    // Detection model information (embedded in binary)
    DetectionModelType detection_model_type_ = DetectionModelType::YUNET;       // Primary: YuNet (embedded)
    DetectionModelType detection2_model_type_ = DetectionModelType::RETINAFACE; // Fallback: RetinaFace (embedded)
    std::string detection_model_name_ = "yunet_2023mar_dynamic";
    std::string detection2_model_name_ = "mnet-retinaface";
    ...
    // Helper: Load embedded detection models from memory
    bool loadEmbeddedYuNet();
    bool loadEmbeddedRetinaFace();
    
    // Detection decoders for YuNet and RetinaFace
