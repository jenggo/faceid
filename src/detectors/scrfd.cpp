...
std::vector<Rect> detectWithSCRFD(ncnn::Net& net, const ncnn::Mat& in, int img_w, int img_h) {
    ...
    // Stride 8 (base_size=16, NOT stride!)
    {
        ...
        generate_scrfd_proposals(anchors, feat_stride, score_blob, bbox_blob, 
    ...
    // Convert to Rect and clip
    std::vector<Rect> faces;
    for (int idx : picked) {
        FaceObject& faceobj = proposals[idx];
        
        float x0 = std::max(std::min((float)faceobj.rect.x, (float)img_w - 1), 0.f);
        float y0 = std::max(std::min((float)faceobj.rect.y, (float)img_h - 1), 0.f);
        float x1 = std::max(std::min((float)faceobj.rect.x + faceobj.rect.width, (float)img_w - 1), 0.f);
        float y1 = std::max(std::min((float)faceobj.rect.y + faceobj.rect.height, (float)img_h - 1), 0.f);
