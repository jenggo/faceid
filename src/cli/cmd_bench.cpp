...
static float findOptimalConfidence(FaceDetector& detector, const ImageView& processed_frame,
    ...
    // Multi-pass search for optimal confidence threshold
    // For low-light IR cameras, we may need to search at very low thresholds
    std::vector<float> search_thresholds = {0.02f, 0.01f, 0.005f, 0.001f};
    float low = -1.0f;
    float high = 0.95f;
    float optimal_confidence = -1.0f;
    int target_face_count = 1;  // We want exactly 1 valid face

    // Multi-pass search: try progressively lower thresholds for dark images
    for (float threshold : search_thresholds) {
        if (countValidFaces(threshold) > 0) {
            low = threshold;
            if (show_progress) {
                std::cout << "  Found faces at threshold=" << std::fixed << std::setprecision(3) << threshold << std::endl;
            }
            break;
        }
    }

    // If no faces detected even at lowest threshold, give up
    if (low < 0) {
        if (show_progress) {
            std::cout << "  No faces detected at any threshold (tried down to 0.001)" << std::endl;
        }
...
int cmd_bench(const std::string& test_dir, bool show_detail, const std::string& custom_image_path) {
    ...
    // Uses multi-pass confidence search to handle low-light images
    {
        ...
        if (!test_detector.loadModels()) {
            ...
            // Multi-pass search for faces (same logic as benchmarks)
            std::vector<float> fallback_thresholds = {0.31f, 0.1f, 0.05f, 0.02f, 0.01f, 0.005f, 0.001f};
            std::vector<Rect> test_faces;
            float used_confidence = 0.0f;
            
            for (float conf : fallback_thresholds) {
                test_faces = test_detector.detectFaces(processed.view(), false, conf);
                if (!test_faces.empty()) {
            ...
            if (test_faces.empty()) {
                ...
                std::cerr << "  2. Image quality is too poor for detection" << std::endl;
                std::cerr << "  3. Face is too small, occluded, or at extreme angle" << std::endl;
                std::cerr << "\nTried confidence thresholds from 0.31 down to 0.001 - all failed." << std::endl;
                std::cerr << "Benchmark will likely show all failures.\n" << std::endl;
            } else {
                std::cout << "Pre-check: Detected " << test_faces.size() << " face(s) with installed model ("
                          << test_detector.getDetectionModelType();
                if (used_confidence != 0.31f) {
                    std::cout << ", confidence=" << std::fixed << std::setprecision(3) << used_confidence;
                }
                std::cout << ")" << std::endl;
            }
        }
    ...
    if (!recognition_models.empty()) {
        ...
        for (auto& model : recognition_models) {
            ...
            // Check if faces are detected - try multiple thresholds for low-light images
            // Note: confidence 0.0 means "use config default", so we use explicit low values
            std::vector<float> fallback_thresholds = {0.31f, 0.1f, 0.05f, 0.02f, 0.01f, 0.005f, 0.001f};
            std::vector<Rect> test_faces;
            float used_confidence = 0.0f;

            for (float conf : fallback_thresholds) {
                test_faces = detector.detectFaces(processed.view(), false, conf);
                if (!test_faces.empty()) {
                    used_confidence = conf;
                    break;
                }
            ...
            if (test_faces.empty()) {
                if (show_detail) {
                    std::cout << "  ✗ No faces detected in test frame (tried thresholds from 0.31 down to 0.001)" << std::endl;
                    std::cout << "     Please position your face in front of the camera and try again." << std::endl;
                }
                continue;
            }
            if (show_detail) {
                std::cout << "  Detected " << test_faces.size() << " face(s) at confidence="
                          << std::fixed << std::setprecision(3) << used_confidence << std::endl;
            }
