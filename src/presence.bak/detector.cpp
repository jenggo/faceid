...
bool PresenceDetector::initialize() {
    ...
    try {
        ...
        // Scan input devices to build monitoring list
        scanInputDevices();
...
void PresenceDetector::updateStateMachine() {
    ...
    switch (current_state_) {
        ...
        case State::IDLE_WITH_SCANNING: {
            // Pre-warm camera on first entry to scanning state to avoid initialization delay
            if (!camera_prewarmed_) {
                std::lock_guard<std::mutex> lock(camera_mutex_);
                if (!camera_ || !camera_->isOpened()) {
                    logger.info("Pre-warming camera to avoid first-scan delay");
                    camera_ = std::make_unique<Camera>(camera_device_);
                    if (camera_->open(presence_camera_width_, presence_camera_height_)) {
                        camera_prewarmed_ = true;
                        logger.info("Camera pre-warmed successfully");
                    } else {
                        logger.warning("Failed to pre-warm camera, will retry on first scan");
                    }
                }
            }
            ...
            if (hasRecentActivity()) {
                ...
                camera_prewarmed_ = false;  // Reset for next scanning cycle
                break;
            }
...
Image PresenceDetector::captureFrame() {
    ...
    if (!camera_ || !camera_->isOpened()) {
        ...
        // IR camera warm-up: Read and discard several frames to let IR illuminator stabilize
        // First frames from IR cameras are often very dark before auto-exposure/IR activates
        logger.info("Warming up IR camera (reading 5 frames to stabilize illuminator)...");
        Image warmup_frame;
        for (int i = 0; i < 5; i++) {
            if (!camera_->read(warmup_frame)) {
                logger.warning("Warm-up frame " + std::to_string(i + 1) + " failed");
                break;
            }
            // Small delay between frames to allow IR to stabilize
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }
        logger.info("IR camera warm-up complete");
    }
...
bool PresenceDetector::hasRecentActivity() const {
    ...
    // Activity detected if idle < 5 seconds (matches cache duration)
    bool activity = (idle_seconds < 5);
...
// NEW: Scan /proc/bus/input/devices to find relevant input event devices
// This detects keyboards, mice, touchpads, touchscreens, and filters out
// non-input devices like audio controls, power buttons, lid switches, etc.
void PresenceDetector::scanInputDevices() {
    Logger& logger = Logger::getInstance();
    
    // Clear existing list
    input_event_devices_.clear();
    ...
    auto process_device = [&]() {
        ...
        // EV_ABS (0x03) = absolute axes (touchpad/touchscreen)
        bool has_key = (current_ev_caps & 0x0001) != 0;  // EV_KEY
        bool has_rel = (current_ev_caps & 0x0002) != 0;  // EV_REL
        bool has_abs = (current_ev_caps & 0x0008) != 0;  // EV_ABS
        ...
        // Format: "H: Handlers=kbd event4 mouse0" or "H: Handlers=event19 mouse5"
        ...
        if (event_pos != std::string::npos) {
            ...
            input_event_devices_.push_back(event_path);
            
            char log_buf[512];
            snprintf(log_buf, sizeof(log_buf), 
                    "Input device found: %s (%s) - EV=0x%x", 
                    event_path.c_str(), current_name.c_str(), current_ev_caps);
    ...
    char log_buf[256];
    snprintf(log_buf, sizeof(log_buf), 
            "Input device scan complete: %zu devices found", 
            input_event_devices_.size());
    logger.info(log_buf);
...
time_t PresenceDetector::getLastInputDeviceActivity() const {
    ...
    // Periodically rescan input devices to detect hotplugged devices (USB/Bluetooth)
    auto time_since_device_scan = std::chrono::duration_cast<std::chrono::minutes>(
        now - last_device_scan_).count();
    if (time_since_device_scan >= device_scan_interval_.count()) {
        // Need to cast away const to update device list
        // This is safe because the method is logically const (doesn't affect observable state)
        const_cast<PresenceDetector*>(this)->scanInputDevices();
    }
    ...
    // Method 2: Check /dev/input/event* device access times
    // This catches ALL input devices including Bluetooth/USB mice that don't
    // show up in /proc/interrupts
    {
        static std::atomic<time_t> last_event_time{0};
        time_t latest_event_time = 0;
        
        for (const std::string& event_path : input_event_devices_) {
            struct stat st;
            if (stat(event_path.c_str(), &st) == 0) {
                // Check both access time and modification time
                // Some systems update atime, others update mtime on device activity
                time_t device_time = std::max(st.st_atime, st.st_mtime);
                
                if (device_time > latest_event_time) {
                    latest_event_time = device_time;
                }
            }
        }
        
        // Check if any event device was accessed/modified since last check
        time_t prev_event_time = last_event_time.load();
        if (latest_event_time > prev_event_time) {
            last_event_time.store(latest_event_time);
            
            char log_buf[256];
            snprintf(log_buf, sizeof(log_buf), 
                    "ACTIVITY DETECTED: Event device time changed (prev=%ld, new=%ld)",
                    prev_event_time, latest_event_time);
            logger.debug(log_buf);
            
            // Update latest_input_time if event device is newer
            if (latest_event_time > latest_input_time) {
                latest_input_time = latest_event_time;
            }
        }
    }
